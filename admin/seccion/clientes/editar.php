<?php 
    ...
    if ($_POST) {
        ...
        if (!preg_match("/^\d{8}$/", $dni)) {
            ...
            $mensajeError = "El número de teléfono debe iniciar con '9' y contener 9 dígitos.";
        ...
        <form id="form_usuario" action="" method="post" enctype="multipart/form-data">
            <div class="row">
                ...
                <div class="col-md-6 col-12 mb-3">
                    ...
                    <input type="file" class="form-control" name="foto" id="foto" accept="image/jpeg, image/png, image/jpg" onchange="previewImage(event)"/>
                </div>

                <div class="col-md-6 col-12 mb-3 d-flex flex-column">
                    <label for="foto" class="form-label">Previsualización de la Foto:</label>
                    <?php
                        $rutaImagen = "../../../images/clientes/" . $usuario['foto'];
                    
                        if (!file_exists($rutaImagen) || empty($usuario['foto'])) {
                            $rutaImagen = "../../../images/default/usuario_default.png";
                        }
                    ?>
                    <img id="preview" src="<?php echo $rutaImagen; ?>" alt="Vista previa de la foto" class="img-fluid rounded border w-25" onclick="toggleImageSize()">
                </div>
            </div>
